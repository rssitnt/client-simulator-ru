...
- The minimal shell now runs behind `body.local-minimal-ui` on `localhost/127.0.0.1` and on the production domains `client-simulator.ru` / `www.client-simulator.ru`:
  ...
  - the localhost prompt-drawer topbar is now its own stacking layer above `.instruction-content`; keep the role trigger/menu above prompt variations so variation labels like `Основной` cannot bleed through under the dropdown;
