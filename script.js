...
const GEMINI_VOICE_ASSISTANT_RESPONSE_WATCHDOG_MAX_RETRIES = 2;
const GEMINI_VOICE_ASSISTANT_RESPONSE_STALL_RECOVERY_MS = 7600;
...
const mainDialogHistoryScopeMeta = document.getElementById('mainDialogHistoryScopeMeta');
const mainDialogHistorySearchInput = document.getElementById('mainDialogHistorySearchInput');
const mainDialogHistoryNewBtn = document.getElementById('mainDialogHistoryNewBtn');
const mainDialogHistoryList = document.getElementById('mainDialogHistoryList');
const mainDialogHistoryLoadMoreBtn = document.getElementById('mainDialogHistoryLoadMoreBtn');
const mainDialogHistoryViewer = document.getElementById('mainDialogHistoryViewer');
const mainDialogHistoryViewerEmpty = document.getElementById('mainDialogHistoryViewerEmpty');
const mainDialogHistoryViewerContent = document.getElementById('mainDialogHistoryViewerContent');
const mainDialogHistoryTitleInput = document.getElementById('mainDialogHistoryTitleInput');
const mainDialogHistoryViewerMeta = document.getElementById('mainDialogHistoryViewerMeta');
const mainDialogHistoryPinBtn = document.getElementById('mainDialogHistoryPinBtn');
const mainDialogHistoryDeleteBtn = document.getElementById('mainDialogHistoryDeleteBtn');
...
const dialogHistoryUiSets = [
    {
        ...
        viewer: mainDialogHistoryViewer,
        viewerEmpty: mainDialogHistoryViewerEmpty,
        viewerContent: mainDialogHistoryViewerContent,
        titleInput: mainDialogHistoryTitleInput,
        viewerMeta: mainDialogHistoryViewerMeta,
        pinBtn: mainDialogHistoryPinBtn,
        deleteBtn: mainDialogHistoryDeleteBtn,
        messages: mainDialogHistoryMessages,
        ratingWrap: mainDialogHistoryRatingWrap,
        ratingText: mainDialogHistoryRatingText
    }
].filter((item) => item.list && item.viewer);
const dialogHistoryTitleInputs = dialogHistoryUiSets.map((item) => item.titleInput).filter(Boolean);
...
let currentDialogHistoryPinnedAt = null;
...
let geminiVoiceAssistantResponseStallRecoveryTimerId = 0;
let geminiVoiceAssistantResponsePendingSince = 0;
...
function resetCurrentSessionToAuth(message = '') {
    ...
    if (mainDialogHistorySearchInput) {
        mainDialogHistorySearchInput.value = '';
    }
...
function buildCurrentDialogHistorySnapshot(options = {}) {
    ...
    const indexRecord = normalizeDialogHistoryIndexRecord({
        ...
        pinnedAt: currentDialogHistoryPinnedAt,
        createdAt,
        updatedAt: nowIso,
...
function renderDialogHistoryList() {
    dialogHistoryUiSets.forEach(renderDialogHistoryListInto);
}

function renderDialogHistoryViewerInto(ui) {
    if (
        !ui?.viewer
        || !ui.viewerEmpty
        || !ui.viewerContent
    ...
    ui.viewerMeta.innerHTML = `
        ...
        <span>${record.messageCount} репл.</span>
        ${record.pinnedAt ? '<span>Закреплён</span>' : ''}
    `;

    if (ui.pinBtn) {
        ui.pinBtn.hidden = !canDeleteSelectedDialogHistory();
        ui.pinBtn.disabled = false;
        ui.pinBtn.textContent = record.pinnedAt ? 'Открепить' : 'Закрепить';
    }
    ui.deleteBtn.hidden = !canDeleteSelectedDialogHistory();
    ...
    if (payload.rating?.text) {
        ui.ratingWrap.hidden = false;
        ui.ratingText.textContent = payload.rating.text;
    } else {
        ui.ratingWrap.hidden = true;
        ui.ratingText.textContent = '';
    }
}
...
async function startFreshDialogFromHistory() {
    ...
    const hasExistingDialog = conversationHistory.length > 0
        ...
        || !!currentDialogHistoryId
        || !!normalizeDialogHistoryText(userInput?.value || '');
    if (hasExistingDialog || isGeminiVoiceConnecting || isGeminiVoiceActive || geminiLiveSession) {
        await clearChat();
    } else {
...
function clearGeminiVoiceAssistantResponseStallRecovery() {
    if (geminiVoiceAssistantResponseStallRecoveryTimerId) {
        clearTimeout(geminiVoiceAssistantResponseStallRecoveryTimerId);
        geminiVoiceAssistantResponseStallRecoveryTimerId = 0;
    }
}

function resetGeminiVoiceAssistantResponseWatchdog() {
    clearGeminiVoiceAssistantResponseWatchdog();
    clearGeminiVoiceAssistantResponseStallRecovery();
    geminiVoiceAssistantResponseWatchdogRetryCount = 0;
    geminiVoiceAssistantResponsePendingSince = 0;
}
...
function scheduleGeminiVoiceAssistantResponseStallRecovery(reason = 'user_turn_finalized') {
    clearGeminiVoiceAssistantResponseStallRecovery();
    if (!isGeminiVoiceActive && !isGeminiVoiceConnecting) return;
    if (!hasPendingGeminiVoiceAssistantResponseGap()) return;

    geminiVoiceAssistantResponseStallRecoveryTimerId = setTimeout(() => {
        geminiVoiceAssistantResponseStallRecoveryTimerId = 0;
        if (!isGeminiVoiceActive && !isGeminiVoiceConnecting) return;
        if (!hasPendingGeminiVoiceAssistantResponseGap()) return;
        const pendingForMs = geminiVoiceAssistantResponsePendingSince
            ? Math.max(0, Date.now() - geminiVoiceAssistantResponsePendingSince)
            : 0;
        sendGeminiVoiceRealtimeActivityMarker('end', `${reason}_stall_recovery`);
        recordVoiceDebugEvent('assistant_response_stall_recovery', {
            status: 'warn',
            message: reason,
            pendingForMs
        });
        showVoiceCallNotice('Клиент задерживается с ответом. Повторно подталкиваю реплику…');
        setVoiceModeStatus('Клиент задерживается с ответом. Повторно подталкиваю реплику…', 'waiting');
        scheduleGeminiVoiceAssistantResponseWatchdog(`${reason}_stall_recovery`);
    }, GEMINI_VOICE_ASSISTANT_RESPONSE_STALL_RECOVERY_MS);
}
...
const VOICE_MODE_STATUS_COPY = Object.freeze({
    ...
    tokenSlowFallback: 'Основной маршрут медленный. Переключаю звонок…',
    dialing: 'Набираем клиента…',
    connecting: 'Соединяем звонок…',
    preparingMic: 'Клиент на линии. Готовлю ваш микрофон…',
    callReady: 'Клиент на линии. Можно говорить.',
    clientSpeaking: 'Клиент говорит…',
    yourTurn: 'Ваша очередь говорить.',
    aiThinking: 'Клиент думает над ответом…',
    active: 'Идёт разговор…',
    rating: 'Оцениваю диалог…',
...
async function resolveGeminiLiveApiKey(sessionConfig = {}, options = {}) {
    ...
    for (let attemptIndex = 0; attemptIndex < endpointCandidates.length; attemptIndex += 1) {
        ...
        if (isFallbackAttempt) {
            setVoiceModeStatus(getVoiceModeStatusCopy('tokenSlowFallback', 'Сервер голосового режима отвечает медленно. Пробуем запасной маршрут…'), 'waiting');
        }
...
async function requestGeminiAssistantFallbackTranscriptForCurrentTurn(reason = 'waiting_for_input') {
    ...
    fetchWithTimeout(endpoint, {
        ...
        .then((payload) => {
            ...
            const mergedTranscript = pickMoreCompleteVoiceTranscript(
                transcript,
                geminiVoiceAssistantPreview
            );
            geminiVoiceHasAssistantReply = true;
            if (turnId <= geminiVoiceAssistantLastFinalizedTurnId) {
                patchGeminiAssistantDialogLineByTurnId(turnId, mergedTranscript || transcript);
                return;
            }
...
function beginGeminiAssistantVoiceOutput(trigger = 'transcript') {
    ...
    showVoiceCallNotice(getVoiceModeStatusCopy('clientSpeaking', 'Клиент отвечает…'));
    setVoiceModeStatus(getVoiceModeStatusCopy('clientSpeaking', 'Клиент отвечает…'), 'waiting', { lockMs: 1800 });
}
...
function scheduleGeminiVoiceMicUnlockAfterPlayback(delayMs = null) {
    ...
    if (!geminiVoiceHasAudioOutput) {
        if (isGeminiVoiceActive) {
            showVoiceCallNotice(getVoiceModeStatusCopy('clientSpeaking', 'Клиент отвечает…'));
            setVoiceModeStatus(getVoiceModeStatusCopy('clientSpeaking', 'Клиент отвечает…'), 'waiting');
        }
        return;
    }
    ...
    geminiVoiceMicUnlockTimerId = setTimeout(() => {
        ...
        showVoiceCallNotice(getVoiceModeStatusCopy('yourTurn', 'Клиент ответил. Теперь можно говорить.'));
        setVoiceModeStatus(getVoiceModeStatusCopy('yourTurn', 'Клиент ответил. Теперь говорите.'), 'listening');
    }, effectiveDelay);
}
...
function looksLikeTruncatedVoiceTranscriptStart(text) {
    const source = normalizeVoiceDialogText(text);
    if (!source) return false;
    const firstChar = source.charAt(0);
    if (!firstChar) return false;
    if (/^[,.;:!?…\-—)\]]$/.test(firstChar)) return true;
...
function sanitizeUserCompletedTranscript(rawText) {
    ...
    if (!source) return '';
    const hasPriorUserTurns =
        geminiVoiceDialogLines.some((line) => line?.role === 'user' && normalizeVoiceDialogText(line?.text))
        || conversationHistory.some((line) => line?.role === 'user' && normalizeVoiceDialogText(line?.content));
    if (!hasPriorUserTurns) return source;
...
function pickMoreCompleteVoiceTranscript(primaryText, secondaryText) {
    const primary = normalizeVoiceDialogText(primaryText);
    const secondary = normalizeVoiceDialogText(secondaryText);
    if (!primary) return secondary;
    if (!secondary) return primary;

    const primaryCompact = normalizeVoiceDialogCompact(primary);
    const secondaryCompact = normalizeVoiceDialogCompact(secondary);
    if (!primaryCompact) return secondary;
    if (!secondaryCompact) return primary;

    if (primaryCompact === secondaryCompact) {
        return pickReadableVoiceVariant(primary, secondary);
    }
    if (primaryCompact.includes(secondaryCompact)) return primary;
    if (secondaryCompact.includes(primaryCompact)) return secondary;
    if (primaryCompact.endsWith(secondaryCompact)) return primary;
    if (secondaryCompact.endsWith(primaryCompact)) return secondary;

    const merged = normalizeVoiceDialogText(mergeVoiceStreamingText(primary, secondary));
    if (merged) {
        const mergedCompact = normalizeVoiceDialogCompact(merged);
        if (mergedCompact.includes(primaryCompact) && mergedCompact.includes(secondaryCompact)) {
            return merged;
        }
    }
...
function pushGeminiVoiceDialogLine(role, text, options = {}) {
    const safeRole = role === 'assistant' ? 'assistant' : 'user';
    const normalizedText = normalizeVoiceDialogText(text);
    if (!normalizedText) return;
    const turnId = Number(options?.turnId || 0) || 0;
    ...
    if (lastLine && lastLine.role === safeRole) {
        ...
        if (previousCompact && currentCompact) {
            if (
                ...
                if (turnId && !lastLine.turnId) {
                    lastLine.turnId = turnId;
                }
                return;
            }
        }
...
function patchGeminiAssistantDialogLineByTurnId(turnId, transcript) {
    const safeTurnId = Number(turnId || 0);
    const normalizedTranscript = normalizeVoiceDialogText(transcript);
    if (!safeTurnId || !normalizedTranscript) return false;

    for (let index = geminiVoiceDialogLines.length - 1; index >= 0; index -= 1) {
        const line = geminiVoiceDialogLines[index];
        if (line?.role !== 'assistant') continue;
        if (Number(line?.turnId || 0) !== safeTurnId) continue;
        const mergedText = pickMoreCompleteVoiceTranscript(line.text, normalizedTranscript) || normalizedTranscript;
        if (normalizeVoiceDialogText(line.text) === mergedText) {
            return false;
        }
...
function appendGeminiVoiceDialogToChat() {
    ...
    let appendedCount = 0;
    let updatedCount = 0;
    for (let index = 0; index < Math.min(geminiVoiceDialogSyncedCount, geminiVoiceDialogLines.length); index += 1) {
        const line = geminiVoiceDialogLines[index];
        const text = normalizeVoiceDialogText(line?.text || '');
        if (!text) continue;

        const chatNode = line?.chatNode;
        const contentNode = chatNode?.querySelector?.('.message-content');
        if (contentNode && contentNode.textContent !== text) {
            contentNode.textContent = text;
            updatedCount += 1;
        }

        const historyIndex = Number.isInteger(line?.historyIndex) ? line.historyIndex : -1;
        if (
            historyIndex >= 0 &&
            historyIndex < conversationHistory.length &&
            conversationHistory[historyIndex]?.content !== text
        ) {
    ...
    for (let index = startIndex; index < geminiVoiceDialogLines.length; index += 1) {
        ...
        const messageNode = addMessage(text, role, false);
        line.chatNode = messageNode;
        appendConversationHistoryEntry({ role, content: text });
        line.historyIndex = conversationHistory.length - 1;
        appendedCount += 1;
    }
...
function finalizeGeminiUserTurn(sourceText) {
    ...
    const mergedUserTurnText = pickMoreCompleteVoiceTranscript(
        mergeVoiceStreamingText(previewText, completedUserText),
        completedUserText
    ) || completedUserText;
    geminiVoiceUserDraft = normalizeVoiceDialogText(
        pickMoreCompleteVoiceTranscript(
            mergeVoiceStreamingText(geminiVoiceUserDraft, mergedUserTurnText),
            mergedUserTurnText
        )
    );
    flushGeminiVoiceDraftLine('user');
    ...
    if (isGeminiVoiceConnecting || isGeminiVoiceActive) {
        geminiVoiceAssistantResponsePendingSince = Date.now();
        ...
        scheduleGeminiVoiceAssistantResponseStallRecovery('user_turn_finalized');
    }
    return true;
}
...
function finalizeGeminiAssistantTurn(sourceText, options = {}) {
    ...
    pushGeminiVoiceDialogLine('assistant', completedAssistantText, {
        turnId: Number(geminiVoiceAssistantCurrentTurnId || 0)
    });
    if (hasBufferedGeminiAssistantAudio() && !geminiVoiceHasAudioOutput) {
...
function handleGeminiVoiceTransportFailure(message = 'Соединение голосового канала прервано. Попробуйте начать звонок заново.') {
    ...
    if (shouldRetryEarlyClose) {
        ...
        stopGeminiVoiceMode({
            ...
            setVoiceModeStatus('Соединение сорвалось. Переподключаем звонок…', 'waiting');
            showVoiceCallNotice('Соединение сорвалось. Переподключаем звонок…');
            setTimeout(() => {
                if (!isVoiceModeScreenActive) {
                    return;
                }
...
async function handleGeminiLiveMessage(message) {
    ...
    if (message.setupComplete) {
        ...
        if (!maybeActivateGeminiVoiceManagerInput()) {
            setVoiceModeStatus(getVoiceModeStatusCopy('preparingMic', 'Клиент на линии. Подготавливаем микрофон…'), 'waiting');
    ...
    if (serverContent?.waitingForInput && isGeminiVoiceActive) {
        ...
        const shouldDelayCompletedAssistant =
            geminiVoicePendingAssistantBeforeFirstUserTurn && !geminiVoiceUserTurnFinalized;
        const shouldPreferFallbackForPendingAssistantTranscript =
            hasPendingAssistantTranscript &&
            !shouldDelayCompletedAssistant &&
            hasBufferedGeminiAssistantAudio() &&
            looksLikeTruncatedVoiceTranscriptStart(pendingAssistantTranscript);
        if (hasPendingAssistantTranscript && !shouldDelayCompletedAssistant && !shouldPreferFallbackForPendingAssistantTranscript) {
            finalizeGeminiAssistantTurn(pendingAssistantTranscript, {
                restoreListeningState: false
            });
            clearGeminiVoiceLateTranscriptWait();
            geminiVoiceAssistantTurnInProgress = false;
        } else if (shouldPreferFallbackForPendingAssistantTranscript) {
            scheduleGeminiVoiceLateTranscriptWait('waiting_for_input');
            void requestGeminiAssistantFallbackTranscriptForCurrentTurn('waiting_for_input').catch(() => {});
            geminiVoiceAssistantTurnInProgress = false;
...
async function startGeminiVoiceMode() {
    ...
    try {
        ...
        setVoiceModeStatus(getVoiceModeStatusCopy('auth', 'Подтверждаем вход…'), 'waiting');
        ...
        geminiLiveSession = await geminiLiveApiClient.live.connect({
            ...
            callbacks: {
                onopen: () => {
                    ...
                    setVoiceModeStatus(getVoiceModeStatusCopy('connecting', 'Соединяем клиента…'), 'waiting');
                },
                onmessage: (message) => {
                    if (startAttempt.id !== geminiVoiceStartAttemptId) return;
        ...
        if (!geminiVoiceSetupComplete) {
            showVoiceCallNotice(getVoiceModeStatusCopy('preparingMic', 'Клиент на линии. Подготавливаем микрофон…'));
        }
    } catch (error) {
...
function updateVoiceModeRateButtonState() {
    ...
    if (isProcessing) {
        setVoiceModeStatus(getVoiceModeStatusCopy('rating', 'Оцениваю диалог…'), 'waiting');
        return;
    }
    if (canShowRate) {
    ...
    if (isGeminiVoiceActive) {
        setVoiceModeStatus(getVoiceModeStatusCopy('active', 'Идёт диалог…'), 'listening');
        return;
    }
}
...
function processElevenLabsRealtimeMessage(message) {
    ...
    if (eventType === 'conversation_initiation_metadata') {
        ...
        resetGeminiVoiceDialogBuffer();
        setVoiceModeStatus(getVoiceModeStatusCopy('active', 'Соединение установлено. Идёт диалог…'), 'listening');
        updateVoiceModeRateButtonState();
        return;
    }
...
function setVoiceModeScreenActive(active) {
    ...
    if (voiceModeScreen) {
        voiceModeScreen.hidden = !active;
        voiceModeScreen.setAttribute('aria-hidden', active ? 'false' : 'true');
    }
    if (voiceModeActions) {
...
dialogHistoryUiSets.forEach((ui) => {
    bindEvent(ui.loadMoreBtn, 'click', () => {
        dialogHistoryVisibleCount += DIALOG_HISTORY_PAGE_SIZE;
        renderDialogHistoryList();
    });

    bindEvent(ui.pinBtn, 'click', () => {
        toggleSelectedDialogHistoryPinned().catch((error) => {
            console.error('Failed to toggle dialog history pin:', error);
            showCopyNotification(error?.message || 'Не удалось изменить закрепление диалога');
        });
    });

    bindEvent(ui.deleteBtn, 'click', () => {
    ...
    bindEvent(ui.titleInput, 'keydown', (event) => {
        if (event.key === 'Escape') {
            event.preventDefault();
            if (dialogHistorySelectedRecord) {
                ui.titleInput.value = dialogHistorySelectedRecord.title
        ...
        event.preventDefault();
        commitDialogHistoryTitleRename(ui.titleInput).catch((error) => {
            console.error('Failed to rename dialog history:', error);
            showCopyNotification(error?.message || 'Не удалось переименовать диалог');
        });
    });

    bindEvent(ui.titleInput, 'blur', () => {
        commitDialogHistoryTitleRename(ui.titleInput).catch((error) => {
            console.error('Failed to rename dialog history:', error);
            showCopyNotification(error?.message || 'Не удалось переименовать диалог');
        });
...
bindEvent(mainDialogHistorySearchInput, 'input', () => {
    dialogHistorySearchQuery = normalizeDialogHistoryText(mainDialogHistorySearchInput.value || '');
    dialogHistoryVisibleCount = DIALOG_HISTORY_PAGE_SIZE;
    renderDialogHistoryScopeMeta();
    renderDialogHistoryList();
});

bindEvent(mainDialogHistoryNewBtn, 'click', () => {
...
function scrollChatToMessage(messageEl, options = {}) {
    ...
    if (shouldAlignStart) {
        const voiceActionsHeight = isVoiceModeScreenActive && voiceModeActions && !voiceModeActions.hidden
            ? Math.max(0, voiceModeActions.offsetHeight || 0)
            : 0;
        const topPadding = voiceActionsHeight > 0 ? (voiceActionsHeight + 20) : 12;
...
function rebuildConversationHistoryTextFromEntries(entries = []) {
    return entries
        .map((entry) => {
            const role = entry?.role === 'user' ? 'user' : 'assistant';
            const content = normalizeDialogHistoryText(entry?.content || '');
            return content ? buildConversationHistoryLine(role, content) : '';
        })
        .filter(Boolean)
        .join('\n\n');
}
...
if (instructionDropdown) {
    const closeInstructionDropdown = () => {
        instructionDropdown.classList.remove('active');
        instructionDropdownTrigger?.setAttribute('aria-expanded', 'false');
    };

    bindEvent(instructionDropdownTrigger, 'click', (e) => {
        e.stopPropagation();
        const nextOpen = !instructionDropdown.classList.contains('active');
        instructionDropdown.classList.toggle('active', nextOpen);
        instructionDropdownTrigger?.setAttribute('aria-expanded', nextOpen ? 'true' : 'false');
    });
    ...
    instructionOptions.forEach(option => {
        option.addEventListener('click', (e) => {
            ...
            const instructionName = option.dataset.label || option.querySelector('.personality-option-title')?.innerText || option.innerText;
            
            // Update UI
            selectedInstructionText.innerText = instructionName;
            instructionOptions.forEach(opt => opt.classList.remove('active'));
            option.classList.add('active');
            closeInstructionDropdown();
