...
- Public share links now use `shared_dialogs` in RTDB:
  ...
  - Firebase rules for `shared_dialogs` are now create-only for signed-in users; existing shared payloads must not be overwritable through the public share path.
- Dialog history rename is now inline in the list:
  ...
  - opening the `...` actions menu now also cancels the delayed row-open timer, so the dialog does not jump open behind the menu after the rename-click guard delay.
