...
## 2026-03-29 — Админская таблица пользователей получила отдельный мобильный layout
- Широкая desktop-таблица `Пользователи и доступ` плохо помещалась в мобильную full-screen шторку: обрезались колонки и строку приходилось читать кусками.
- Исправление:
  - в `script.js` у ячеек таблицы появились `data-label` (`Логин`, `Роль`, `Доступ`, `Активное время`, `Статус`, `Действия`);
  - в `style.css` на мобильном thead превращается в компактный row только из sort-кнопок;
  - строки tbody на телефоне больше не рендерятся как широкие table-rows, а как stacked cards с подписями полей;
  - action-кнопки внутри строки растягиваются на всю ширину карточки;
  - для light theme добавлены отдельные mobile overrides, чтобы карточки не оставались тёмными.

## 2026-03-29 — Мобильные настройки переведены в full-screen и получили свой крестик закрытия
- На телефоне плавающая кнопка настроек могла визуально налезать на открытую модалку и конфликтовать с контентом.
- Исправление:
  - `settingsModal` на мобильном теперь растягивается на весь экран (`100vw / 100dvh`) без внешних отступов и скруглённых углов;
  - сама шторка использует safe-area padding внутри, а не снаружи;
  - плавающая кнопка настроек скрывается, пока `#settingsModal` открыт;
  - внутри настроек добавлен отдельный крестик закрытия, чтобы мобильный сценарий не зависел от внешней плавающей кнопки.
- Дополнительная доводка:
  - крестик больше не живёт в потоке первой строки и не сдвигает цветовой ряд;
  - на мобильном он закреплён сверху справа как fixed-control и остаётся доступным даже при скролле в самый низ настроек;
  - верхнему блоку настроек добавлен запас по правому краю и сверху, чтобы крестик не наезжал на цветовые контролы.

## 2026-03-29 — Второй проход по мобильным настройкам: увеличены слишком мелкие элементы
- После первого редизайна mobile settings sheet структура стала лучше, но часть контролов на телефоне всё ещё была слишком мелкой для чтения и нажатия.
- Второй проход в `style.css` поднял именно размеры, не меняя архитектуру модалки:
  - увеличены theme toggle и цветовые пресеты;
  - увеличены `name input`, logout, экспорт, роль-действия;
  - voice/microphone picker получили более высокий trigger и более крупную типографику;
  - увеличены тексты и bubbles в `История диалогов`;
  - увеличены admin controls: invite fields, hidden prompt summary/textarea, password input и confirm/cancel/change actions.
- Изменения ограничены mobile breakpoint (`max-width: 768px`), desktop не тронут.

## 2026-03-29 — Мобильные настройки переделаны в более правильный sheet-layout
- Старый мобильный вид настроек выглядел как desktop-форма, просто ужатая по ширине: подписи слева, контролы справа, тесные строки и слабая иерархия.
- Исправление в `style.css`:
  - `#settingsModal` на мобильных теперь выравнивается как верхний sheet внутри safe area, а не как случайно центрированный блок;
  - `.settings-panel` получил более сильную sheet-подачу: больше радиус, граница, тень и более плотную внутреннюю ритмику;
  - базовые settings rows стали мобильными карточками со stacked-структурой: label сверху, контент ниже;
  - `Экспорт` на мобильных теперь stacked full-width;
  - голос и микрофон получили более крупные tap targets;
  - экшены аккаунта/роли стали более touch-friendly.
- Для light theme добавлены отдельные mobile overrides, чтобы новый мобильный layout не был качественным только в тёмной теме.

## 2026-03-29 — Мобильные крайние иконки выровнены по центру относительно `Чат / Роль`
- На мобильном виде иконки очистки чата и настроек сидели чуть ниже, чем визуальный центр кнопок `Чат` и `Роль`.
- Исправление в `style.css`:
  - внутри mobile breakpoint (`max-width: 768px`) уменьшен `top` для `.btn-clear-floating` и `.btn-settings-floating`.
- Десктоп не тронут; изменена только мобильная позиция крайних иконок.

## 2026-03-29 — Клик по `История` у админа теперь прокручивает к реальному блоку истории
- После фикса Firebase rules и retry токена мог появляться новый UX-баг: `Permission denied` уже не было, но по клику на `История` визуально не происходило ничего.
- Реальная причина была не в чтении данных, а в разметке/скролле модалки:
  - блок `История диалогов` расположен выше `Панели администратора`;
  - клик по `История` только ставил `open` на accordion, но не прокручивал settings modal к этому месту.
- Исправление в `script.js`:
  - добавлен `revealDialogHistoryAccordion(...)`;
  - он открывает accordion, скроллит его в видимую зону и на короткое время подсвечивает блок.
- Исправление в `style.css`:
  - добавлена краткая highlight-подсветка и `scroll-margin-top` для `dialogHistoryAccordion`.

## 2026-03-29 — Чужая история теперь автоматически ретраится после refresh Firebase token
- После публикации новых history-rules у админа мог оставаться `Permission denied` при открытии чужой истории не потому, что ветка пустая, а потому что браузер ещё жил со старым Firebase auth token / stale RTDB auth state.
- Исправление в `script.js`:
  - `fetchDialogHistoryScopeRecords(...)` и `fetchDialogHistoryPayload(...)` теперь при первом `permission denied` один раз делают `auth.currentUser.getIdToken(true)` и повторяют чтение.
  - если после refresh доступ всё ещё закрыт, пользователь получает уже не голое `Permission denied`, а понятное сообщение про необходимость обновить вход / подождать после публикации rules.
- Важное уточнение:
  - пустая история не должна давать deny; при пустой ветке UI показывает пустой список.

## 2026-03-29 — Ошибка оценки больше не маскирует проблему записи в историю
- После внедрения постоянной истории диалогов выяснилось, что пользователь мог получать красную `Ошибка оценки: PERMISSION_DENIED`, хотя сама оценка уже была успешно получена из webhook.
- Реальная причина: `rateChat(...)` после получения рейтинга сразу делал `saveCurrentDialogHistoryNow(...)`, и ошибка RTDB по новым history-rules падала в общий `catch` оценки.
- Исправление:
  - сохранение оценки в историю теперь обёрнуто в отдельный `try/catch`;
  - если ломается только история, рейтинг остаётся на экране и диалог считается оценённым;
  - пользователь получает отдельное сообщение вида `Оценка получена, но не сохранилась в историю...`, а не ложную `Ошибка оценки`.
- Дополнительно:
  - то же разделение применено к отправке аттестационного отчёта, чтобы вторичный сбой не откатывал уже успешную оценку.
- Важный операционный вывод:
  - для полной работы истории всё ещё нужно опубликовать новые rules из `C:/projects/sites/client-simulator/database.rules.json` в Firebase Console.

## 2026-03-29 — Постоянная история диалогов + честное active time
- В настройки добавлена постоянная история диалогов на Firebase RTDB:
  - индекс: `dialog_history_index/{loginKey}/{dialogId}`;
  - содержимое: `dialog_history_messages/{loginKey}/{dialogId}`.
- Хранятся только текстовые данные:
  - обычные чат-сообщения,
  - голосовые транскрипты,
  - оценка как отдельный блок `{ text, createdAt }`.
- Аудиозаписи не сохраняются.
- История создаётся лениво только после первой реальной реплики, а не по кнопке старта.
- Автоназвание берётся из первой осмысленной реплики клиента; если её нет, используется fallback по времени.
- Пользователь теперь может:
  - видеть список своих диалогов в настройках,
  - открывать детали,
  - переименовывать свои названия inline,
  - удалять свои диалоги.
- Админ теперь может открыть историю любого пользователя прямо из `Пользователи и доступ` по кнопке `История`:
  - просмотр и удаление разрешены,
  - переименование чужих диалогов намеренно запрещено.
- Учёт active time стал честнее:
  - фокус окна, возврат на вкладку и сам факт логина больше не считаются активностью;
  - время идёт только при `visible + focused + было реальное действие за последние 60с`;
  - действиями считаются `pointerdown`, `keydown`, `touchstart`, `input`, `change`, `paste`, `scroll`, `wheel`;
  - при blur/hidden накопление останавливается и не возобновляется без нового реального действия.
- Для истории добавлены RTDB rules:
  - владелец читает/пишет только свою историю;
  - админ читает любую историю и может удалять любой диалог.
...
- Старое поведение:
  - при `waitingForInput` менеджерский mic быстро открывался обратно,
  - если текст первой реплики ещё не приехал, в чат нечего было финализировать,
  - в итоге первая реплика могла остаться только в аудио и не попасть в историю как отдельная bubble.
- Исправление в `script.js`:
  - добавлен grace window `GEMINI_VOICE_LATE_TRANSCRIPT_GRACE_MS = 650`,
  - при `waitingForInput` без готового assistant transcript, но с уже начавшимся assistant audio, включается `geminiVoiceAwaitingLateAssistantTranscript`,
  - unlock микрофона менеджера откладывается на это окно,
  - если `outputTranscription` или `part.text` догоняют позже, они сразу финализируются в чат как завершённая первая реплика.
- Регрессия закрыта в `scripts/smoke-e2e.mjs` новым сценарием `late-first-transcript`:
  - сначала приходит audio,
  - потом `waitingForInput`,
  - и только потом delayed `outputTranscription`,
  - smoke проверяет, что первая реплика AI-клиента всё равно появляется в чате.
