...
- Inline rename in history now keeps the delayed-open guard scoped correctly: pressing `...` after the first click must cancel the pending open timer, otherwise the record opens underneath the actions menu after ~260ms. Smoke now covers that menu-vs-open race directly.
