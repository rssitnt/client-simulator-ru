...
- the local admin panel density was tightened again: on desktop-like widths, user cards are now compact two-column rows with the login spanning full width and the single access button tucked to the bottom-right; the invite row and accordion headers are smaller too;
...
- the localhost `Роль` drawer width was increased by about 10% from the earlier baseline, so prompt editing has a bit more horizontal space;
- local settings sections are flattened now: the outer section wrappers no longer appear as separate rounded rectangles and instead use subtle divider lines;
...
- the left history rail scrollbar should now be styled by the same minimal rule shape as settings itself: the scroll lives on `.history-panel`, spans the full panel height, and the inner history list must not create its own nested scrollbar.
...
- the admin role dropdown in `Пользователи и доступ` now opens as a floating fixed menu and closes on scroll/resize, so it should no longer be clipped by the compact drawer/table wrapper;
- closing the localhost role drawer now also force-closes the inner role dropdown, so the drawer should not reopen in a half-broken “dropdown still active / prompt content hidden” state;
