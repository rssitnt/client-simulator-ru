...
- The token-server rate-limit bucket now stores `resetAt` again, and `getRetryAfterMs()` stays top-level:
  - otherwise a real 429 path could fail inside the server instead of returning the structured `rate_limited` payload with `Retry-After`.
...
- Mobile history controls are now hard-aligned directly under the app bar with a fixed top offset and matching body padding to remove the extra gap.
