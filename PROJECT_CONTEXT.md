...
- Verification on `2026-04-16`:
  - `scripts/integration-smoke.mjs` was repaired after Firebase auth/app-check drift:
    - the auth stub now exports `sendPasswordResetEmail`;
    - App Check is now stubbed too, so the integration run no longer crashes on missing Firebase internals like `_getProvider`;
    - the hidden rater prompt setup in integration smoke is written directly into `raterHiddenPrompt:v1` instead of depending on whichever settings button is visible in the current shell.
  - integration smoke now also appends compact `pageErrors / consoleErrors / debugState` diagnostics on failure so future regressions are easier to triage.
  - `npm run test:smoke` and `npm run test:smoke:integration` both pass after this repair.
