...
## 2026-03-29 — Найден и закрыт корень потери первого слышимого ответа Gemini Live
- Реальная причина была не в рендере чата и не в декодировании первого чанка как таковом.
- Во время первого ответа AI-клиента микрофон менеджера оставался открытым:
  - `maybeActivateGeminiVoiceManagerInput()` включал mic сразу после `setupComplete`,
  - но при начале `outputTranscription` / первом аудиочанке mic не выключался,
  - из-за этого Gemini продолжал получать входной звук во время собственного ответа и мог присылать `interrupted` до того, как первый ответ реально становился слышимым.
- Исправление в `script.js`:
  - добавлен `beginGeminiAssistantVoiceOutput(trigger)`,
  - он вызывается на первом `outputTranscription`, `modelTurn.parts[].text`, `inlineData` и `outputAudio`,
  - при старте ответа сразу делает half-duplex: выключает mic менеджера, пишет это в voice debug log и переводит статус в `Клиент отвечает…`,
  - mic возвращается только через `scheduleGeminiVoiceMicUnlockAfterPlayback()` после фактического завершения воспроизведения.
- Дополнительно:
  - состояние `geminiVoiceAssistantTurnInProgress` сбрасывается на финализации ответа, `waitingForInput`, reset/stop.
- Ожидаемый эффект:
  - первая реплика AI-клиента должна стать слышимой, а не обрываться собственной же сессией из-за открытого микрофона менеджера.
- После правки прошли:
  - `node --check script.js`
  - `npm run test:smoke`

## 2026-03-29 — Починена потеря первой реплики клиента при прерывании Gemini Live
- Найден реальный path потери первой реплики AI-клиента в звонке:
  - реплика копилась в `geminiVoiceAssistantPreview`,
  - но в чат финализировалась только на `outputFinished` / `turnComplete`,
  - если менеджер начинал говорить раньше или Gemini присылал `interrupted`, эта первая реплика оставалась только во временном preview и терялась как отдельная фраза.
- Исправление в `script.js`:
  - добавлен отдельный `finalizeGeminiAssistantTurn(...)`,
  - он вызывается не только на нормальном завершении хода, но и при первом новом `inputTranscription` менеджера, если у клиента уже есть незавершённый preview/draft,
  - а также на `serverContent.interrupted`.
- Итог:
  - прерванная первая реплика клиента теперь фиксируется в чате как отдельная реплика,
  - она больше не должна пропадать между первым и вторым клиентским ответом.
- После правки прошли:
  - `node --check script.js`
  - `npm run test:smoke`
- В `scripts/smoke-e2e.mjs` добавлен отдельный Gemini voice smoke-сценарий `assistant-interrupted-first-reply`:
  - он симулирует первый клиентский ответ без `turnComplete`,
  - затем `interrupted`,
  - и проверяет, что эта первая реплика всё равно попадает в чат.

## 2026-03-28 — Добавлен техлог Gemini Live для админа
- В настройки (админский блок) добавлен accordion `Техлог голосового режима`.
- Это локальный лог текущего браузера, не серверный.
- В него пишутся ключевые этапы голосовой сессии:
  - старт звонка,
  - загрузка SDK,
  - запрос token endpoint и его успех/ошибка,
  - открытие Gemini Live,
  - `setupComplete`,
  - готовность микрофона,
  - первый звук менеджера,
  - первый текст клиента,
  - первый аудиочанк клиента,
  - старт первого воспроизведения,
...
- Есть кнопки:
  - `Скопировать техлог`
  - `Очистить`
- Цель: быстро понимать, где именно ломается звонок, без слепого поиска по симптомам.

## 2026-03-28 — Починен ложный статус «Идёт подключение…» на входе
- Это был не реальный самозапуск звонка, а сломанный `hidden` у блока статуса подключения.
- Причина: `.voice-connect-status` задавал `display: flex` и перебивал нативное скрытие элемента с атрибутом `hidden`.
- Добавлено правило `.voice-connect-status[hidden] { display: none !important; }`.
- Итог: плашка подключения видна только во время настоящего старта голосового звонка.

## 2026-03-28 — Добавлен выбор реального микрофона и live-чек громкости на гудках
- В настройках рядом с выбором голоса добавлен отдельный picker микрофона для Gemini Live.
- Логика списка микрофонов:
  - запрашивается/обновляется через `navigator.mediaDevices.enumerateDevices()`,
  - берутся только `audioinput`,
  - выкидываются `default` / `communications`,
  - убираются безымянные фантомные записи,
  - дубли режутся по `label + groupId`.
- Выбранный `deviceId` сохраняется локально и подставляется в `getUserMedia` через `deviceId.exact`.
- Если ранее выбранный микрофон пропал, фронт автоматически сбрасывает сохранённый `deviceId`, переключается на доступный вход и обновляет picker.
- Пока идут гудки/подключение, в `voiceConnectStatus` теперь показывается живая шкала уровня микрофона:
  - красный — недостаточно,
  - жёлтый — достаточно,
  - зелёный — хорошо.
- Уровень считается прямо из текущего входного аудиопотока до того, как менеджеру открывается полноценный разговорный режим.
